namespace JoF.Rail.Standard.Profiles
{
    using System;
    using AutoMapper;
    ...
    using ServiceType = JoF.Rail.Standard.Enums.ServiceType;
    using FilterTYpe = JoF.Rail.Standard.Enums.FilterType;
    ...
    public class LiveDeparturesMappingProfile : Profile
    {
        public LiveDeparturesMappingProfile()
        {
            ...
            this.CreateMap<ServiceItemWithCallingPoints1, ServiceItemModel>(MemberList.Destination)
                ...
                .ForMember(dest => dest.Id, opt => opt.MapFrom(src => this.RandomNumber()))
                .ForMember(dest => dest.IsCancelled, opt => opt.MapFrom(src => src.isCancelled))
